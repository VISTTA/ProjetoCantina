...
exports.loginUser = catchAsyncErrors ( async (req,res,next) => {
    ...
    if(!email || !password) {
        return next(new ErrorHander ("Por favor insira a o email e senha.", 400));
    }
    ...
    if(!isPasswordMatched) {
        return next(new ErrorHander ("Email ou senha incorreto.", 401));
    }
...
exports.updatePassword = catchAsyncErrors(async(req, res, next) => {
    ...
    if(!isPasswordMatched) {
        return next( new ErrorHander("Antigo email invalido", 400));
    };
    if(req.body.newPassword !== req.body.confirmPassword) {
        return next( new ErrorHander("A senha não confere.", 400));
    }
...
exports.getSingleUser = catchAsyncErrors (async (req, res, next) => {
    ...
    if(!user) {
        return next( new ErrorHander (`Usuário com o seguinte id não existe: ${req.params.id}`));
    }
